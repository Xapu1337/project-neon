...
import {Event} from "../Miscellaneous/Events";
import Utility from "../Miscellaneous/Utilities";
...
export class NeonClient extends Client {
    ...
    public async run( ) {
        ...
        await klaw(path.join(__dirname, "../../events")).on("data", async (item) => {
            ...
            this.utilities.println(`Successfully bound event ${chalk.bgBlack.white`${eventFile.name}`} to ${item.path}`, chalk.bgWhite.black`EVENT BIND`);
        });
